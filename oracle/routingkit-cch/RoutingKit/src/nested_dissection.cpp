...
BlockingFlow::BlockingFlow(
	...
	// Allocate scratch buffers once. compute_blocking_flow and
	// augment_all_non_blocked_path reset rather than reallocate.
	bfs_is_on_same_level_or_lower(fragment.node_count(), BitVector::uninitialized),
	bfs_was_node_pushed(fragment.node_count(), BitVector::uninitialized),
	bfs_queue(fragment.node_count()),
	aug_path_node(fragment.node_count()),
	aug_path_arc(fragment.node_count())
{
	#ifndef NDEBUG
	assert_fragment_is_valid(fragment);
...
namespace{
	// File-static accumulators for the inertial-flow scaling investigation.
	// Reset and emitted by compute_nested_node_dissection_order_using_inertial_flow.
	// Atomic to remain safe if OpenMP customization runs in parallel; the
	// nested-dissection order computation itself is single-threaded.
	std::atomic<long long> g_total_advance_calls{0};
	std::atomic<long long> g_total_advance_micros{0};
	std::atomic<long long> g_total_cut_decisions{0};

	void reset_inertial_flow_stats(){
		g_total_advance_calls.store(0);
		g_total_advance_micros.store(0);
		g_total_cut_decisions.store(0);
	}

	void emit_inertial_flow_stats(const std::function<void(const std::string&)>&log_message){
		if(!log_message) return;
		long long calls = g_total_advance_calls.load();
		long long micros = g_total_advance_micros.load();
		long long cuts = g_total_cut_decisions.load();
		long long avg_per_cut = (cuts > 0) ? (calls / cuts) : 0;
		long long avg_micros_per_call = (calls > 0) ? (micros / calls) : 0;
		log_message(
			"InertialFlow totals: cut_decisions=" + std::to_string(cuts) +
			" advance_calls=" + std::to_string(calls) +
			" advance_micros=" + std::to_string(micros) +
			" avg_calls_per_cut=" + std::to_string(avg_per_cut) +
			" avg_micros_per_call=" + std::to_string(avg_micros_per_call)
		);
	}
	...
	bool compute_blocking_flow(const GraphFragment&fragment, const BitVector&is_source, const BitVector&is_target, const BitVector&is_arc_saturated, BitVector&is_arc_blocked,
		BitVector&is_on_same_level_or_lower, BitVector&was_node_pushed, std::vector<unsigned>&queue){
		// Scratch buffers are owned by the calling BlockingFlow; reset rather
		// than reallocate. Allocator-reuse patch (2026-06-06).
		is_on_same_level_or_lower.reset_all();
		was_node_pushed.reset_all();
	...
	unsigned augment_all_non_blocked_path(const GraphFragment&fragment, const BitVector&is_source, const BitVector&is_target, BitVector&is_arc_saturated, BitVector&is_arc_blocked,
		std::vector<unsigned>&current_path_node, std::vector<unsigned>&current_path_arc){
		// Scratch buffers owned by the calling BlockingFlow — reset semantically
		// via the loop logic below; no zeroing required because every entry is
...
CutSide inertial_flow(
	...
	// Per-call counters accumulate into the file-static totals. No per-call
	// log emission — keeps the log volume bounded; the summary is emitted
	// once at the end of compute_nested_node_dissection_order_using_inertial_flow.
	long long local_advance_calls = 0;
	long long local_advance_micros = 0;
	...
	for(;;){
		...
		if(!c.is_finished()){
			...
			// Flush this inertial_flow invocation's totals into the file-static
			// accumulator. One add per inertial_flow call — negligible overhead
			// compared to the work just done.
			g_total_advance_calls.fetch_add(local_advance_calls, std::memory_order_relaxed);
			g_total_advance_micros.fetch_add(local_advance_micros, std::memory_order_relaxed);
			g_total_cut_decisions.fetch_add(1, std::memory_order_relaxed);
			return cut; // NRVO
		}
	}
}
...
std::vector<unsigned>compute_nested_node_dissection_order_using_inertial_flow(
	...
	// Investigation hook: reset the file-static inertial-flow stats so a
	// single order computation reports its own totals. Emitted via
	// log_message at the end (one summary line). No per-cut emission.
	reset_inertial_flow_stats();
	...
	auto order = compute_nested_node_dissection_order(g, compute_separator, log_message);

	// One summary line per order computation. Useful for the scaling study;
	// silent in production (no log_message wired) and tiny when enabled.
	emit_inertial_flow_stats(log_message);
