...
private:
	...
	// Scratch buffers reused across advance() calls — sized once in the
	// constructor, reset (not reallocated) per call. Previously these were
	// fresh allocations inside compute_blocking_flow + augment_all_non_blocked_path
	// on every advance, which dominates allocator pressure at country scale.
	BitVector bfs_is_on_same_level_or_lower;
	BitVector bfs_was_node_pushed;
	std::vector<unsigned> bfs_queue;
	std::vector<unsigned> aug_path_node;
	std::vector<unsigned> aug_path_arc;
public:
	BlockingFlow(){}
